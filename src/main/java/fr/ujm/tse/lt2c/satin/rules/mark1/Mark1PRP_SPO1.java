...
public class Mark1PRP_SPO1 extends AbstractRule {
	...
	public Mark1PRP_SPO1(Dictionnary dictionnary, TripleStore usableTriples,
			Collection<Triple> newTriples, TripleStore tripleStore,
			CountDownLatch doneSignal) {
		super(dictionnary, tripleStore, usableTriples, newTriples, "PRP_SPO1",
				doneSignal);
	...
	public void run() {
		...
		/*
		 * If usableTriples is null, we infer over the entire triplestore
		 */
		...
		/*
		 * If usableTriples is not null, we infer over the matching triples
		 ...
		 */
			...
			for (Triple t1 : usableTriples.getAll()) {
				...
				for (Triple t2 : p1 == subPropertyOf ? tripleStore.getAll()
						: subPropertyOf_Triples) {
