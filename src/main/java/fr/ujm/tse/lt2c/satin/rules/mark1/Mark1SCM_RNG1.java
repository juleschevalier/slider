		...
		if (subclassMultimap != null && subclassMultimap.size() > 0) {
			...
			for (Triple triple : rangeTriples) {
				...
				for (Long c2 : c2s) {
					...
					logTrace(dictionary.printTriple(new TripleImplNaive(triple.getSubject(), range, triple.getObject())) + " & " + dictionary.printTriple(new TripleImplNaive(triple.getObject(), subClassOf, c2)) + " -> " + dictionary.printTriple(result));
				}
			}
		}
