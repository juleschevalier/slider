 ...
 * p rdfs:range c2
 */
public class Mark1mtSCM_RNG1 extends AbstractRuleMT {

	private static Logger logger = Logger.getLogger(Mark1mtSCM_RNG1.class);

	public Mark1mtSCM_RNG1(Dictionnary dictionnary, TripleStore usableTriples,  Collection<Triple> newTriples, TripleStore tripleStore) {
		super();
		this.dictionnary = dictionnary;
		this.tripleStore = tripleStore;
		this.usableTriples = usableTriples;
		this.newTriples = newTriples;
		this.ruleName = "SCM_RNG1";
	}
			...
			for (Triple t1 : range_Triples) {
				...
				for (Triple t2 : subClassOf_Triples) {
					...
					if(o1==s2){
						...
						logTrace("F SCM_RNG1 "+dictionnary.printTriple(t1)+" & "+dictionnary.printTriple(t2)+" -> "+dictionnary.printTriple(result));
