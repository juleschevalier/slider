...
import fr.ujm.tse.lt2c.satin.slider.reasoner.BatchReasoner;
...
public final class Main {
    ...
    private static final Ruleset DEFAULT_PROFILE = IncrementalReasoner.DEFAULT_PROFILE;
    ...
    private static RunEntity reasonStream(final ReasoningArguments arguments, final File file) {
        ...
        if (arguments.isVerboseMode()) {
            ...
            for (final RuleModule ruleModule : reasoner.getRules()) {
                rules.add(ruleModule.name());
            }
            final RunEntity run = new RunEntity(arguments.getThreadsNb(), arguments.getBufferSize(), arguments.getTimeout(), "0.9.7", arguments.getProfile().toString(), rules,
                    UUID.randomUUID().hashCode(), file.getName(), 0, stop - start, inputSize, tripleStore.size() - inputSize, GlobalValues.getRunsByRule(),
                    GlobalValues.getDuplicatesByRule(), GlobalValues.getInferedByRule(), GlobalValues.getTimeoutByRule());
            GlobalValues.reset();
            return run;
        }
        GlobalValues.reset();
    ...
    private static RunEntity reasonBatch(final ReasoningArguments arguments, final File file) {
        final TripleStore tripleStore = new VerticalPartioningTripleStoreRWLock();
        final Dictionary dictionary = new DictionaryPrimitrivesRWLock();
        final BatchReasoner reasoner = new BatchReasoner(tripleStore, dictionary, arguments.getProfile(), arguments.getThreadsNb(), arguments.getBufferSize(),
        ...
        if (arguments.isVerboseMode()) {
            ...
            final RunEntity run = new RunEntity(arguments.getThreadsNb(), arguments.getBufferSize(), arguments.getTimeout(), "0.9.6", arguments.getProfile().toString(), rules,
                    UUID.randomUUID().hashCode(), file.getName(), start - parse, stop - start, triples.size(), tripleStore.size() - triples.size(), GlobalValues.getRunsByRule(),
                    GlobalValues.getDuplicatesByRule(), GlobalValues.getInferedByRule(), GlobalValues.getTimeoutByRule());
            GlobalValues.reset();
            return run;
    ...
    private static ReasoningArguments getArguments(final String[] args) {
        ...
        int iteration = 1;
        Ruleset profile = DEFAULT_PROFILE;
        ...
        final Option profileO = new Option("p", "profile", true, "set the fragment " + java.util.Arrays.asList(Ruleset.values()));
        ...
        if (cmd.hasOption("profile")) {
            ...
            switch (string.toLowerCase()) {
                case "rhodf":
                    profile = Ruleset.RHODF;
                    break;
                case "brhodf":
                    profile = Ruleset.BRHODF;
                    break;
                case "rdfs":
                    profile = Ruleset.RDFS;
                    break;
                case "brdfs":
                    profile = Ruleset.BRDFS;
                    break;
                case "lrdfs":
                    profile = Ruleset.LRDFS;
                    break;
